...
namespace PadelQ.Application.Common.Interfaces
{
    public interface IBookingService
    {
        ...
        Task<(bool Success, string Message, Guid BookingId)> CreateAdminBooking(string? userId, string? guestName, string? guestPhone, string? guestEmail, string? dni, int courtId, DateTime startTime, int durationMinutes, decimal depositPaid = 0);
        Task<(bool Success, string Message, List<Guid> BookingIds)> CreateRecurringBooking(string? userId, string? guestName, string? guestPhone, string? guestEmail, string? dni, int courtId, DateTime startTime, int durationMinutes, DateTime endDate, decimal depositPaid = 0);
